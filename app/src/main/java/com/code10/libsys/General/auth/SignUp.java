...
public class SignUp extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        EditText enterLibrarianId = findViewById(R.id.etenterLibraryId);
        ...
        selectRole.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> adapterView, View view, int position, long l) {
                ...
                if (userRole.equals("Librarian")) {
                    enterLibrarianId.setVisibility(View.VISIBLE);
                } else {
                    enterLibrarianId.setVisibility(View.INVISIBLE);
                }
            }
        ...
        imageBack.setOnClickListener(v -> startActivity(new Intent(SignUp.this, GetStarted.class)));
        ...
        signup.setOnClickListener(view -> {
            ...
            String libid = enterLibrarianId.getText().toString();

            if (userRole.equals("")) {
                Toast.makeText(this, "Select Role", Toast.LENGTH_SHORT).show();
            } else {
                if (!userRole.equals("Librarian")) {
                    checkvalidityAndcreateAcc(email, enterPassword, conformPassword);
                } else {
                    if (Idcheck(libid)) {
                        checkvalidityAndcreateAcc(email, enterPassword, conformPassword);
                    } else {
                        Toast.makeText(this, "Librarian Id is not valid", Toast.LENGTH_SHORT).show();
                    }
                }
    ...
    private boolean Idcheck(String libid) {
        if (libid.toLowerCase().startsWith("lib") && libid.length() == 7) {
            char[] chars = libid.toLowerCase().replace("lib", "").toCharArray();
            for (char c : chars) {
    ...
    private void checkvalidityAndcreateAcc(String email, String enterPassword, String conformPassword) {
